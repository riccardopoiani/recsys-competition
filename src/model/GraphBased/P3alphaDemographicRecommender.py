...
class P3alphaDemographicRecommender(BaseRecommender):
    """ P3 alpha DemographicRecommender """

    RECOMMENDER_NAME = "P3alphaDemographicRecommender"

    def __init__(self, URM_train, UCM_train, verbose=True):
        super(P3alphaDemographicRecommender, self).__init__(URM_train, verbose=verbose)
    ...
    def fit(self, topK=100, alpha=1., min_rating=0, implicit=False, normalize_similarity=False):
        ...
        self.W_sparse = sps.csr_matrix((values[:numCells], (rows[:numCells], cols[:numCells])),
                                       shape=(self.n_items + self.UCM_train.shape[1],
                                              self.n_items + self.UCM_train.shape[1]))
    ...
    def _compute_item_score(self, user_id_array, items_to_compute=None):
        ...
        :return:
        """
        URM_UCM = sps.hstack([self.URM_train, self.UCM_train], format="csr")
        user_profile_array = URM_UCM[user_id_array]
        ...
        if items_to_compute is not None:
            ...
            item_scores = user_profile_array.dot(self.W_sparse).toarray()[:, :self.n_items]
