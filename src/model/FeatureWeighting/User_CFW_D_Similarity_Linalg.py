...
class User_CFW_D_Similarity_Linalg(BaseUserSimilarityMatrixRecommender):
    ...
    def __init__(self, URM_train, UCM, S_matrix_target):
        ...
        if (URM_train.shape[0] != UCM.shape[0]):
            raise ValueError(
                "Number of users not consistent. URM contains {} but UCM contains {}".format(URM_train.shape[1],
                                                                                             UCM.shape[0]))

        if (S_matrix_target.shape[0] != S_matrix_target.shape[1]):
            raise ValueError(
                "User similarity matrix is not square: rows are {}, columns are {}".format(S_matrix_target.shape[0],
