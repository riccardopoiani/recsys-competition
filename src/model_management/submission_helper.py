...
# TODO: to be tested
def write_submission_file_batch(recommender, path, userlist, epochs=10):
    '''
    :param recommender: tells how to recommend item for the users. It must be a
    recommender class
    :param path: where to store the model
    :param userlist: list of users to recommend. The list get sorted in the function
    so you don't need to do it before
    :param epochs: number of epochs to recommend user list
    :return: none
    '''
    from tqdm import tqdm
    import numpy as np
    ...
    f = open(path, "w+")
    f.write("user_id,item_list\n")
    user_split_lists = np.array_split(userlist, epochs)
    for i in tqdm(range(len(user_split_lists))):
        ...
        for j in range(recommendation_list):
            f.write(str(userlist[i * len(users) + j]))
            f.write(",")
            for item in recommendation_list[j]:
                f.write(str(item))
                f.write(" ")
