...
from course_lib.Data_manager.DataSplitter import DataSplitter
from course_lib.Data_manager.DataReader_utils import compute_density, reconcile_mapper_with_removed_tokens
from course_lib.Data_manager.Split_functions.split_train_validation_leave_k_out import split_train_leave_k_out_user_wise
from course_lib.Data_manager.data_consistency_check import assert_disjoint_matrices, assert_URM_ICM_mapper_consistency


class New_DataSplitter_leave_k_out(DataSplitter):
    """
    ...
    - The "original" subfolder contains the whole dataset, is composed user_demographic a single URM with all data and may contain
    ...
    DATA_SPLITTER_NAME = "NewDataSplitter_leave_k_out"
    ...
    def __init__(self, dataReader_object, k_out_value=1, forbid_new_split=False, force_new_split=False,
                 use_validation_set=True, leave_random_out=True, allow_cold_users=False, seed=None):
        """
    ...
    def get_loaded_UCM_dict(self):
        if self.allow_cold_users:
            return self.dataReader_object.get_loaded_UCM_dict()
    ...
    def get_holdout_split(self):
        ...
        :return: URM_train, URM_validation, URM_test if !self.use_validation_set, otherwise URM_train, URM_test
        """
