...
def plot_metric_results_by_user_activity(results: list, metric_name: str = "MAP", cutoff: int = 10,
                                         URM_train: sps.csr_matrix = None):
    """
    Plot one metric from the dict of metric results generated by evaluate_recommender_by_user_activity

    :param results: list of dicts of metric results generated by evaluate_recommender_by_user_activity
    :param metric_name: the name of the metric to plot
    :param URM_train: if URM_train is passed, the plot will have also the plot of user activity
    :return: None
    """
    metrics = _get_metric_result(results, metric_name, cutoff)
    bins = len(metrics)

    max_ID = URM_train.shape[0] if URM_train is not None else 100
    ...
    plt.suptitle("Barplot of {} for different subset of user by user activity".format(metric_name))
    ...
    if URM_train is not None:
        ...
        user_activity_train = np.sort(user_activity_train)
        ax2 = plt.twinx()
        ax2.plot(user_activity_train, 'r.', label='User activity')
