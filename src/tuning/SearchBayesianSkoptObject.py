...
from course_lib.ParameterTuning.SearchAbstractClass import SearchAbstractClass, writeLog, \
...
class SearchBayesianSkoptObject(SearchAbstractClass):
    ALGORITHM_NAME = "SearchBayesianSkoptObject"

    # Value to be assigned to invalid configuration or if an Exception is raised
    INVALID_CONFIG_VALUE = np.finfo(np.float16).max

    def __init__(self, recommender_object, evaluator_validation=None, evaluator_test=None):
        super(SearchBayesianSkoptObject, self).__init__(recommender_object,
    ...
    def _evaluate_on_validation(self, current_fit_parameters):
        ...
        recommender_instance = self.recommender_class.copy()
