...
from src.tuning.cross_validation.CrossSearchAbstractClass import CrossSearchAbstractClass


class CrossSearchBayesianSkopt(CrossSearchAbstractClass):
    ALGORITHM_NAME = "CrossSearchBayesianSkopt"

    def __init__(self, recommender_class, evaluator_validation_list=None, verbose=True):

        assert evaluator_validation_list is not None, "{}: evaluator_validation must be provided".format(
            self.ALGORITHM_NAME)

        super(CrossSearchBayesianSkopt, self).__init__(recommender_class,
