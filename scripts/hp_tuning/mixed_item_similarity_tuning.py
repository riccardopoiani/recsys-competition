...
if __name__ == '__main__':
    ...
    # Path setting
    print("Start tuning...")
    version_path = "../../report/hp_tuning/mixed/"
    now = datetime.now().strftime('%b%d_%H-%M-%S')
    now = now + "_k_out_value_3_eval/"
    version_path = version_path + now

    item_cf = best_models.ItemCF.get_model(URM_train, load_model=True)
    item_cbf_cf = best_models.ItemCBF_CF.get_model(URM_train=URM_train, load_model=True, ICM_train=ICM_subclass_all)
    item_cbf_all = best_models.ItemCBF_all.get_model(URM_train=URM_train, ICM_train=ICM_all, load_model=True)
    ...
    run_parameter_search_mixed_similarity(hybrid, URM_train=URM_train, output_folder_path=version_path,
                                          ...
                                          n_cases=35, n_random_starts=10, metric_to_optimize="MAP")
